...
async function onUpload() {
  ...
  console.log('Time taken to generate MD5', performance.now() - start, 'ms');

  const clip = await videoDecoder.decode({ id, stream: files[0].stream(), type: files[0].type });

  console.log('Time taken to parse video', performance.now() - start, 'ms');

  if (!clip) {
    // 提示解析视频失败
    ElMessage.error('Failed to parse video');
    return;
  }
